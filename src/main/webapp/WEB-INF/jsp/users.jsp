...
<c:if test="${not empty requestScope.users}">
    <c:forEach var="book" items="${requestScope.users}">
        <div>
            <c:if test="${sessionScope.user.id eq book.id}"><b><fmt:message key="you"/></b></c:if>
            <fmt:message key="login"/> ${book.login},
            <fmt:message key="role"/> ${book.role},
            <fmt:message key="subscription"/> ${book.subscription}
